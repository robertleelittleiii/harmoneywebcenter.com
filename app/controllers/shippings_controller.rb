class ShippingsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @shipping.save
        ...
        format.json { render :json=>@shipping.errors, :status=>:unprocessable_entry }
      end
    end
  end
  ...
  def destroy
   ...
   # CREATE_EMPTY_RECORD /shippings/1
   # CREATE_EMPTY_RECORD /shippings/1.json

  def create_empty_record
    @shipping = Shipping.new
    @shipping.save
    
    redirect_to(:controller=>:shippings, :action=>:edit, :id=>@shipping)
  end
